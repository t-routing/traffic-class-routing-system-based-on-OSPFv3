...
ospf6_intra_traffic_class_calculation (struct ospf6_area *oa)
{
  ...
  void (*hook_add) (struct ospf6_tcr_route *) = NULL;
  void (*hook_remove) (struct ospf6_tcr_route *) = NULL;
  ...
  hook_add = oa->tcr_route_table->hook_add;
  hook_remove = oa->tcr_route_table->hook_remove;
  oa->tcr_route_table->hook_add = NULL;
  oa->tcr_route_table->hook_remove = NULL;

  ospf6_mark_tcr_route_remove (oa->tcr_route_table);

  type = htons (OSPF6_LSTYPE_INTRA_TCR);

  //ospf6_tcr_route_table_delete_nodes (oa->tcr_route_table);

  for (lsa = ospf6_lsdb_type_head (type, oa->lsdb); lsa;
       lsa = ospf6_lsdb_type_next (type, lsa))
    ospf6_intra_tcr_lsa_add (lsa);

  oa->tcr_route_table->hook_add = hook_add;
  oa->tcr_route_table->hook_remove = hook_remove;

  for (ALL_LIST_ELEMENTS_RO(oa->tcr_route_table->table->tcr_route_node_list, node, trn))
    {
      ...
      if (CHECK_FLAG (tcr_route->flag, OSPF6_ROUTE_REMOVE) &&
          CHECK_FLAG (tcr_route->flag, OSPF6_ROUTE_ADD))
        {
          UNSET_FLAG (tcr_route->flag, OSPF6_ROUTE_REMOVE);
          UNSET_FLAG (tcr_route->flag, OSPF6_ROUTE_ADD);
        }

      if (CHECK_FLAG (tcr_route->flag, OSPF6_ROUTE_REMOVE))
        ospf6_tcr_route_remove (tcr_route, oa->tcr_route_table);
      else if (CHECK_FLAG (tcr_route->flag, OSPF6_ROUTE_ADD) ||
               CHECK_FLAG (tcr_route->flag, OSPF6_ROUTE_CHANGE))
        {
          if (hook_add)
            (*hook_add) (tcr_route);
        }
